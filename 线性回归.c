...
/* void creat_data(Point *sample, int f); //传入Point数组首位指针，位数，自动生成线性回归样本
float cost_function(Point *sample, int l, float k, float a);//传入Point数组首位指针，位数，斜率，截距，返回cost
...
int gradient_descent(Point *sample,int l, float *k, float *a,float alpha,Cost *max3,int i){
    ...
    a2 = *a-alpha*get_sum_a(sample, l, k, a);
    k2 = *k-alpha*get_sum_k(sample, l, k, a);
    *a = a2;
    *k = k2;
    //利用max3_increase更新
